...
$$
\mathsf{a} \triangleq \langle \mathsf{Id}(c_i), j \rangle \in
...
$$
\forall \mathsf{d}_i \in \mathcal{D}(\mathsf{a}) : 
\mathsf{d}_i \triangleq \langle \sigma, \alpha, \mathcal{L} \rangle
\in \mathbb{D} \triangleq \mathbb{V} \times \mathbb{F}_q \times \{ \varnothing, \ell \}
$$
...
$$
\forall \mathsf{e}_i \in \mathcal{I}(\mathsf{a}) :
\mathsf{e}_i \triangleq \langle \sigma, \mathcal{R} \rangle
\in \mathbb{I} \triangleq \mathbb{V} \times \{ \varnothing, \mathbb{N}_8^{[0; 2^{16})} \}_\preceq
$$
...
(\exists \ y \in \mathcal{Y}_i: y_\mathsf{addr} \in \mathcal{A}_j)
\vee
(\exists \ x \in \mathcal{X}_i: x_\mathsf{addr} \in \mathcal{B}_j)
$$
...
TBD: Peter Todd, Giacomo Zucco, Adam Borko
